...
if pioutil.is_pio_build():
    ...
    def xor_data_buffer(pbuf, pkey):
        return bytes(pbuf[i] ^ pkey[i % len(pkey)] for i in range(len(pbuf)))
    
    def swap_data_buffer(pdata, swap_table):
        return bytes(swap_table[data] for data in pdata)
    
    def shuffle_data_buffer(pdata):
        return bytes(pdata[4*j + i] for i in range(4) for j in range(4))
    ...
    def generate_xor_table(key):
      key_len = len(key)
      xor_key_table = bytearray(176)
      xor_key_table[:key_len] = key.encode()
      for i in range(key_len, 176):
          xor_key_table[i] = xor_key_table[i - 4] ^ xor_key_table[i - key_len]
      return xor_key_table
    
    def encrypt_data_block(pdata, xor_key_table):
      pdata = xor_data_buffer(pdata, xor_key_table[:16])
      for i in range(1, 11):
          pdata = swap_data_buffer(pdata, swap_table_encode)
          pdata = shuffle_data_buffer(pdata)
          pdata = xor_data_block(pdata)
          pdata = xor_data_buffer(pdata, xor_key_table[16*i:16*(i+1)])
      return pdata
    
    def encrypt_file(input_file, output_file, key):
       print("Encrypting Marlin Firmware binary %s to file %s with encryption key: %s\n" % (input_file, output_file, key))
       ...
       encrypted_data = encrypt_data_block(data,xor_key_table)
